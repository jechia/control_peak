...
# Usage:
#   bash genAnno.sh --gtf <gtf_file> --rna-list <rna_list_file> [--out-dir <output_dir>] [--prefix <output_prefix>]
#
# Description:
...
# ------------------ Usage ------------------
usage() {
    echo "Usage: $0 --gtf <gtf_file> --rna-list <rna_list_file> [--out-dir <output_dir>] [--prefix <output_prefix>]"
    echo ""
    echo "Arguments:"
    ...
    echo "  --prefix     Prefix for output filenames (default: gencode_v46)"
    exit 1
}
...
awk -v OFS="\t" '$3=="transcript" { match($0, /transcript_name "([^"]+)"/, a); print $1, $4-1, $5, a[1], $6, $7 }' "$GTF" > "$OUT_DIR/${PREFIX}_transcripts.bed"
awk -v OFS="\t" '$3=="transcript" { match($0, /gene_name "([^"]+)"/, a); print $1, $4-1, $5, a[1], $6, $7 }' "$GTF" > "$OUT_DIR/${PREFIX}_genes.bed"
...
awk -v OFS="\t" '$3=="stop_codon" {
    ...
    print $1, s, e, $20, 0, $7
}' "$GTF" | tr -d '";' > "$OUT_DIR/${PREFIX}_stop_codon.bed"

# ----------- Step 2: Get UTR regions -----------
echo "Step 2: Extracting UTRs..."
awk -v OFS="\t" '$3=="UTR"{print $1, $4, $5, $20, 0, $7, $16}' "$GTF" | tr -d '";' > "$OUT_DIR/${PREFIX}_UTR.bed"

# ----------- Step 3: Annotate UTRs as 5' or 3' -----------
echo "Step 3: Classifying UTRs as 5'/3'..."
awk -v OFS="\t" '
BEGIN {
    while ((getline < "'"$OUT_DIR/${PREFIX}_stop_codon.bed"'") > 0) {
        start[$4] = $2; end[$4] = $3
    }
}
{
...
}' "$OUT_DIR/${PREFIX}_UTR.bed" > "$OUT_DIR/${PREFIX}_UTR_anno.bed"

# ----------- Step 4: Extract CDS -----------
echo "Step 4: Extracting CDS..."
awk -v OFS="\t" '$3=="CDS"{print $1, $4, $5, $20"_CDS_"$4, 0, $7, $16}' "$GTF" | tr -d '";' > "$OUT_DIR/${PREFIX}_CDS.bed"

# ----------- Step 5: Combine CDS and UTR -----------
echo "Step 5: Combining CDS and UTR annotations..."
cat "$OUT_DIR/${PREFIX}_CDS.bed" "$OUT_DIR/${PREFIX}_UTR_anno.bed" > "$OUT_DIR/${PREFIX}_pc_trans_anno.bed"
...
awk -v OFS="\t" '
BEGIN {
    while ((getline < "'"$RNA_LIST"'") > 0) {
        biotype[$1] = 1
    }
}
$7 in biotype {
    print $1, $2, $3, $4, $5, $6, $8
}' > "$OUT_DIR/${PREFIX}_exons_RNA.bed"

# ----------- Step 7: Extract protein coding transcript names -----------
echo "Step 7: Generating transcript name list..."
cat "$OUT_DIR/${PREFIX}_exons_RNA.bed" "$OUT_DIR/${PREFIX}_pc_trans_anno.bed" | awk '{split($4, a, "_"); print a[1]}'  | sort | uniq > "$OUT_DIR/${PREFIX}_trans_name.list"

# ----------- Step 8: Filter transcript BED -----------
awk 'BEGIN{while((getline<"'"$OUT_DIR/${PREFIX}_trans_name.list"'")>0){d[$1]=1}}$4 in d' "$OUT_DIR/${PREFIX}_transcripts.bed" > "$OUT_DIR/${PREFIX}_filtered_transcripts.bed"
...
awk -v OFS="\t" '
BEGIN {
    while ((getline < "'"$OUT_DIR/${PREFIX}_trans_name.list"'") > 0) {
        trans[$1] = 1
    }
}
$4 in trans {
    print $1, $2, $3, $4"_exon_"$2, $5, $6, $8
}' > "$OUT_DIR/${PREFIX}_exons.bed"

# ----------- Step 10: Final annotation file -----------
echo "Step 9: Creating final annotation file..."
cat "$OUT_DIR/${PREFIX}_exons.bed" "$OUT_DIR/${PREFIX}_pc_trans_anno.bed" > "$OUT_DIR/${PREFIX}_anno.bed"

echo "✅ Done. Final annotation is at:"
echo "$OUT_DIR/${PREFIX}_anno.bed"
